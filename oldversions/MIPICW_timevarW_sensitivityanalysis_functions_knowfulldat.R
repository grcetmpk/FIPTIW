...
gendataMIPI_timevar_SENS <- function(n, beta1, beta2, beta3, 
  ...
  #### NOTE: we are not removing the censored observations but marking them as censored. This is a test
  #### to make sure that IPCW works in this setting, although we would not have this full dataset in 
  #### practice
  ...
  colnames(simdatafull) <- c("id", "time", "G", "W", "Z", "censortime", 
                             "cexp_W_G", "cexp_Z_G",  "y", "notcensored")
  ...
  while(id <= n){
    ...
    if(length(obstimes) > 0){
      ...
      simdata$notcensored <- 1
      simdata$notcensored[which(simdata$time > simdata$censortime)] <- 0
  ...
  # indicator for subject having an event (whether or not it was observed)
  simdata$event <- 1
...
simulateOneMIPI_timevar_SENS <- function(n, beta1, beta2, beta3, 
  ...
  # IPCw Weights (Currently not working)
  # kappa.hat <- coxph(Surv(time.lag, time, notcensored) ~ G + W + Z - 1, data = simdata)$coef
  # ipcw <- 1/(exp(cbind(simdata$G, simdata$W, simdata$Z)%*%kappa.hat))
  
  ipcwmodel <- pec::ipcw(Surv(time, notcensored)~G+W+Z,
                         data=simdata,
                         method="cox",
                         times=sort(unique(simdata$time)),
                         subjectTimes=simdata$time)
  kappa.hat <- ipcwmodel$fit$coefficients
  ipcw <- ipcwmodel$IPCW.subjectTimes
  ...
  out <- list(beta1,  beta1_naive,  beta1_iiw,  beta1_iiw_stab, 
              beta1_ipw, beta1_ipw_stab, beta1_ipcw,
              beta1_mipiw, beta1_mipiw_stab, beta1_mipicw)
...
simulateResultsMIPI_timevar_SENS <-  function(N, n, beta1, beta2, beta3, gamma1, gamma2, gamma3, 
  ...
  results_beta1 <- matrix(data = NA, nrow = N, ncol = 9)
  ...
  for(i in 1:N){
    ...
    results_beta1[i,] <- c(simrun$beta1_naive, simrun$beta1_iiw, simrun$beta1_iiw_stab,
                           simrun$beta1_ipw, simrun$beta1_ipw_stab, simrun$beta1_ipcw,
                           simrun$beta1_mipiw, simrun$beta1_mipiw_stab, simrun$beta1_mipicw) 
  }
  
  
  
  bias_beta1 <- round(apply(results_beta1, FUN =  mean, MARGIN = 2)  - beta1 ,3)
  names(bias_beta1) <- c("Naive", "IIW", "IIW Stabilized", "IPW", "IPW Stabilized", "MIPIW", "MIPIW Stabilized", "MIPICW")
  
  var_beta1 <- round(apply(results_beta1, FUN = var, MARGIN = 2), 3)
  names(var_beta1) <- c("Naive", "IIW", "IIW Stabilized", "IPW", "IPW Stabilized", "MIPIW", "MIPIW Stabilized", "MIPICW")
  
  mse_beta1 <- round(apply((results_beta1 - beta1)^2, FUN = mean, MARGIN = 2), 3)
  names(mse_beta1) <- c("Naive", "IIW", "IIW Stabilized", "IPW", "IPW Stabilized", "MIPIW", "MIPIW Stabilized", "MIPICW")
  ...
  IPCW_beta1 <-c(bias_beta1[6], var_beta1[6], mse_beta1[6])
  names(IPCW_beta1) <- c("Bias", "Var", "MSE")
  
  MIPIW_beta1 <- c(bias_beta1[7], var_beta1[7], mse_beta1[7])
  names(MIPIW_beta1) <- c("Bias", "Var", "MSE")
  
  MIPIWstab_beta1 <- c(bias_beta1[8], var_beta1[8], mse_beta1[8])
  names(MIPIWstab_beta1) <- c("Bias", "Var", "MSE")
  
  
  MIPICW_beta1 <- c(bias_beta1[9], var_beta1[9], mse_beta1[9])
  names(MIPICW_beta1) <- c("Bias", "Var", "MSE")
  
  out <- list(bias_beta1, var_beta1, mse_beta1, 
              naive_beta1,  IIW_beta1,  IIWstab_beta1, IPW_beta1, IPWstab_beta1, 
              IPCW_beta1,
              MIPIW_beta1, MIPIWstab_beta1, MIPICW_beta1)
...
###did not update past here yet

simulateALLMIPI_timevar_SENS <- function(N, n, beta1, beta2, beta3, gamma1, gamma2vec, gamma3, 
  ...
  resultsmat <- matrix(NA, nrow = 1, ncol = 19)
    ...
    for(kappa1 in kappa1vec){
      for(kappa2 in kappa2vec){
        for(kappa3 in kappa3vec){
          ...
          print(paste("Now on gamma2 = ", gamma2, 
                      ", kappa1 = ", kappa1, 
                      ", kappa2 =  ", kappa2, 
                      ", kappa3 = ", kappa3,
                      sep = ""))
          ...
          resultsmat <- rbind(resultsmat, c(gamma2,  kappa1, kappa2, kappa3,
                                            result$naive_beta1, result$IIW_beta1, 
                                            result$IPW_beta1, result$MIPIW_beta1, result$MIPICW_beta1))
  ...
  colnames(resultsmat) <- c( "gamma2",  "kappa1", "kappa1", "kappa3", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE",
