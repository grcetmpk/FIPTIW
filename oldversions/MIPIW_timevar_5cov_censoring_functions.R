...
gendataMIPI_timevar_5cov_censoring <- function(n, beta1, beta2, beta3, beta4, beta5,
  ...
  # Treatment assignment: P(G_i = 1) = expit(alpha0 + alpha1W_i1 + alpha2W_i2) 
  #
  # Censoring hazard: h(t) = h_0*exp(kappa1D_i + kappa2W_i1 + kappa3W_i2 + kappa4Z_i1 + kappa5Z_i1)
  #
  # tau: maximum follow up time, independent censoring.
  # N: number of simulations
...
simulateOneMIPI_timevar_5cov_censoring <- function(n, beta1, beta2, beta3, beta4, beta5,
  ...
  singlerun <- gendataMIPI_timevar_5cov_censoringl(n, beta1, beta2, beta3, beta4, beta5,
                                        gamma1, gamma2, gamma3, gamma4, gamma5,
                                        kappa1, kappa2, kappa3, kappa4, kappa5,
                                        alpha0, alpha1, alpha2, tau)
  ...
  # IIW Weights
  gamma.hat <- coxph(Surv(time.lag, time, visit) ~ G + tt(W1) +W2 +Z1 + tt(Z2) - 1, 
                     tt = function(x, t, ...) x*log(t), data = simdata, timefix = FALSE)$coef
...
simulateALLMIPI_timevar_5cov_censoring <- function(N, n, beta1, beta2, beta3, beta4, beta5,
  ...
  for(alpha1 in alpha1vec){
    for(alpha2 in alpha2vec){
      for(gamma2 in gamma2vec){
        for(gamma3 in gamma3vec){
          
          
          
          print(paste("Now on alpha1 = ", alpha1, 
                      ", alpha2 =  ", alpha2, 
                      ", gamma2  = ", gamma2,
                      ", gamma3 = ", gamma3,
                      sep = ""))
          ...
          resultsmat <- rbind(resultsmat, c(alpha1, alpha2, gamma2, gamma3,
                                            result$naive_beta1, result$IIW_beta1, 
                                            result$IPW_beta1, result$MIPIW_beta1))
          
          listname <- paste("fulldata_alpha1_", alpha1, "_alpha2_", alpha2, "_gamma2_", 
                            gamma2, "_gamma3_", gamma3, sep = "")
  ...
  colnames(resultsmat) <- c( "alpha1", "alpha2", "gamma2", "gamma3", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE", 
                             "Bias", "Var" , "MSE")
